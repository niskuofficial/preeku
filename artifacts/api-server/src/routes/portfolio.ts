...
router.get("/portfolio/positions", requireAuth, async (req, res) => {
  ...
  try {
    const positions = await db.select().from(positionsTable)
      .where(and(eq(positionsTable.userId, userId), eq(positionsTable.productType, "INTRADAY")));
    ...
    res.json(positions.map((p) => {
      ...
      return { id: p.id, symbol: p.symbol, stockName: p.stockName, quantity: p.quantity, avgBuyPrice, currentPrice, pnl, pnlPercent, currentValue, investedValue, productType: p.productType };
    }));
  } catch (err) {
...
router.get("/portfolio/holdings", requireAuth, async (req, res) => {
  const userId = (req as any).userId;
  try {
    const holdings = await db.select().from(positionsTable)
      .where(and(eq(positionsTable.userId, userId), eq(positionsTable.productType, "DELIVERY")));
    if (holdings.length === 0) { res.json([]); return; }
    ...
    res.json(holdings.map((h) => {
      ...
      return { id: h.id, symbol: h.symbol, stockName: h.stockName, quantity: h.quantity, avgBuyPrice, currentPrice, pnl, pnlPercent, currentValue, investedValue, dayChange, dayChangePercent };
    }));
  } catch (err) {
